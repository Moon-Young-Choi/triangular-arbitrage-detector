...
test("performance budget separates latency domains and marks cross-clock metrics", () => {
  ...
  assert.equal(summary.dashboard.estimatedEndToDisplayMs.clockSkewSensitive, true);
});
...
test("performance budget reports p50 p95 p99 by latency domain", () => {
  ...
  assert.equal(report.dashboardLatencyAffectsTrading, false);
  ...
  assert.equal(report.dashboard.serverToClientMs.p95Ms, 300);
  assert.equal(report.dashboard.serverToClientMs.tradingDecisionInput, false);
  assert.equal(report.marketData.exchangeToSocketMs.clockSkewSensitive, true);
});

test("trading latency budget ignores dashboard render latency", () => {
  ...
  assert.equal(ok.dashboardLatencyAffectsTrading, false);
  assert.deepEqual(ok.ignoredDomains, ["dashboard"]);
