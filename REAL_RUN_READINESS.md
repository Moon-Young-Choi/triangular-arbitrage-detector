...
The engine refreshes private REST permission, account, and `orders/chance` state before evaluating the checklist. The result is written to append-only events and appears in the dashboard Settings tab with a readiness score. Dry-run evidence is not a real-run gate; it remains available for reports and operator review only.
...
Readiness, dry-run, and real-run evidence is stored as normalized audit events. Each appended row carries `auditSchemaVersion` plus `auditSchema.ok` and `auditSchema.missingRequiredFields`. Dry-run reports can be filtered by period and include market-state, latency-band, and best-level touch-ratio breakdowns, so stale/unavailable/available conditions and fragile liquidity/latency zones can be reviewed separately without blocking real-run startup.

The replay CLI regenerates the dry-run review summary from saved orderbook tapes by rebuilding strategy decision rows and executing accepted plans through the same dry-run executor. This keeps replay evidence aligned with the dashboard report without touching Upbit order APIs.
...
Dry-run report thresholds under `executionPolicy.readinessGuards` are legacy configuration fields. They may still load for compatibility, but `REAL_GUARDED` and `REAL_AUTO` no longer use dry-run report quality, start-asset samples, simulated completion rate, rejection rate, or expected-vs-simulated PnL gap as startup gates.
...
- `Q_GAGARIN_LIVE_TRADING_ENABLED=true` enables `liveTradingEnabled` when starting `REAL_GUARDED` from the CLI/runtime command path; dry/observe starts force it back to false.
